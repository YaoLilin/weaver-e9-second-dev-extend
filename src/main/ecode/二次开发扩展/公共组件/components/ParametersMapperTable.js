...
class ParametersMapperTable extends React.Component {
    ...
    componentDidMount() {
        ...
        const dataSource = this.normalizeDataSource(this.props.dataSource);
        if (dataSource === 'mode') {
    ...
    componentDidUpdate(prevProps) {
        const prevSource = this.normalizeDataSource(prevProps.dataSource);
        const currentSource = this.normalizeDataSource(this.props.dataSource);
        const dataSourceChanged = prevSource !== currentSource;
        const workflowChanged = prevProps.workflowId !== this.props.workflowId;
        const modeChanged = prevProps.modeId !== this.props.modeId;

        if (dataSourceChanged || workflowChanged || modeChanged) {
        ...
        if (dataSourceChanged && currentSource !== 'mode') {
            this.loadSystemParams();
        }
    }
            ...
            <div>
                {JsonParametersTable ?
                    <JsonParametersTable
                        ...
                        onChange={onChange}
                        readonly={true}
                        resetFieldValuesOnTypeChange={{ assignment: null, detailNum: '' }}
                                ...
                                render: (text, record) => {
                                    ...
                                    return (
                                        <Select
                                            ...
                                            placeholder="请选择"
                                            allowClear
                                            onChange={(v) => handleChange(record.id, 'detailNum', v || null)}
                                        >
...
if (PropTypes) {
    ParametersMapperTable.propTypes = {
        ...
        /** 字段来源 */
        dataSource: PropTypes.oneOf(['workflow', 'mode', 'model']),
        /** 参数数据 */
